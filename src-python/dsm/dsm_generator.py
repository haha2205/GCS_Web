    ...
    def _load_raw_data(
        ...
        if os.path.exists(fcs_path):
            try:
                ...
                # 1. 解析时间戳并排序
                # 2. 前向填充(ffill)处理稀疏数据
                # 3. 重采样(Resample)到50Hz (20ms)，解决高频录制带来的行数膨胀和错位问题
                if 'timestamp' in df_fcs.columns:
                    # 自动推断时间格式，通常是 "YYYY-MM-DD HH:MM:SS.ffffff"
                    df_fcs['timestamp'] = pd.to_datetime(df_fcs['timestamp'])
                    df_fcs.sort_values('timestamp', inplace=True)
                    
                    df_fcs.set_index('timestamp', inplace=True)
                    
                    # ffill: 将上一行的有效值填充到当前行 (解决UDP分包导致的稀疏列)
                    df_fcs = df_fcs.ffill()
                    
                    # Resample: 按20ms窗口聚合，取该窗口内最后一条完整数据 (解决频率过高问题)
                    # dropna(how='all'): 如果某20ms窗口内完全没有数据（录制间隙），则丢弃，避免产生全NaN行
                    df_fcs = df_fcs.resample('20ms').last().dropna(how='all')
                    
                    # 剩余并未填充的NaN (如文件开头的空缺) 填0
                    df_fcs.fillna(0, inplace=True)
                    
                    # 恢复 timestamp 列为秒级浮点数 (兼容后续逻辑)
                    df_fcs.reset_index(inplace=True)
                    # 转换为 Unix Timestamp (Seconds)
                    df_fcs['timestamp'] = df_fcs['timestamp'].astype(np.int64) / 10**9
                
                logger.info(f"加载FCS遥测数据: {len(df_fcs)} 条记录 (已执行Resample@50Hz)")
